...
INSERT INTO turma VALUES(NEXTVAL('seq_turma'), (SELECT id FROM curso WHERE nome = 'Análise e Desenvolvimento de Sistemas' AND modalidade = 'Tecnólogo'), 2, 'B', 32, '2016-2');
...
INSERT INTO turma VALUES(NEXTVAL('seq_turma'), (SELECT id FROM curso WHERE nome = 'Sistemas de Informação' AND modalidade = 'Bacharel'), 6, 'A', 10, '2016-2');
...
SELECT reserva.id, curso.modalidade, curso.nome AS curso, laboratorio.numero AS laboratorio, software.fabricante, software.nome AS software, reserva.professor, turma.semestre, turma.turma, reserva.tipo FROM curso, reserva, laboratorio, software, turma WHERE reserva.turma = turma.id AND laboratorio.id = reserva.laboratorio AND reserva.softwares = software.id AND curso.id = reserva.curso AND reserva.professor = 'ptoledo';